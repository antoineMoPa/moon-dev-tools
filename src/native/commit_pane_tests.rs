...
/// Several tasks can be working in one repo, and the branch says which of them this commit is.
///
/// Both lines here resolve to the same working copy - the branch neither of them is about is
/// checked out nowhere - so the repo alone cannot tell them apart, and the tasks are read in name
/// order, which has nothing to do with which one is being worked on.
#[test]
fn the_line_for_the_branch_this_repo_is_on_is_the_one_that_fills_the_box() {
