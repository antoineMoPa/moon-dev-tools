//! What a server offers while a name is being written or pointed at: the definition a ⌘-click
//! jumps to, the words that finish the one being typed, and the signature of the call.
...
use crate::native::ui_tests::{
    Fixture, app_for, click_at, click_like_a_hand, press_key, press_modifiers, settle, type_letter,
};
...
fn a_command_clicked_name_in_a_file_no_server_serves_says_so_and_jumps_nowhere() {
    ...
    click_like_a_hand(&mut harness, word, egui::Modifiers::COMMAND);
