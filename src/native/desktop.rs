...
#[cfg(target_os = "macos")]
use super::text_without_a_key;
use super::{Launch, app, logos};
use crate::{
    api::OpenSessionRequest,
    backend::{local::LocalBackend, remote::RemoteBackend},
    server,
};
...
pub(crate) fn launch_local(open: OpenSessionRequest, frame: crate::cli::Frame) -> Result<Launch> {
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            if let Err(error) = runtime.block_on(server::serve(served_state)) {
...
/// Review a repo on another machine through its `moonreview serve`, let in with `pass_key`.
pub(crate) fn launch_remote(
    ...
    let backend = RemoteBackend::connect(target, pass_key)?;
