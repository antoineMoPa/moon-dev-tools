...
/// `~/.moonreview/settings.json` on the machine the project is on, so it follows the project
/// to a window on any machine: which color a window is belongs to the person, not the repo.
