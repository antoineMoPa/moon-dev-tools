//! The files themselves: reading them into rows, and writing a row's change back.
...
pub(crate) fn list_for_repo(repo_path: &Path) -> Vec<ReviewRequestView> {
    ...
    // The lines of a board mostly name the same repo on the same branch, and where that is
    // and how much is changed there is three or four git runs to find out - once per board
    // read, not once per line. See `reviewed_by`.
...
fn view_of(
    ...
    let Reviewed {
        ...
        .entry((request.path_under_repo.clone(), request.branch.clone()))
        .or_insert_with(|| reviewed_by(repo_path, &request))
        .clone();
...
/// The work a line is about: where it is, what to call it, and how much of it is uncommitted.
#[derive(Clone)]
struct Reviewed {
    name: String,
    path: PathBuf,
...
/// Find the work a line names, which is what git is asked for a row.
///
...
fn reviewed_by(repo_path: &Path, request: &ReviewRequest) -> Reviewed {
    ...
    let path = working_copy_of(&repo, request.branch.as_deref());
    Reviewed {
        name,
        ...
        // be read there.
        changed_files: crate::git::changed_file_count(&path).unwrap_or(1),
        path,
    }
}
