//! The server's side of review comments: the agents they are sent to, and the export of them.
...
use super::{ANCHOR_CLOSE, ANCHOR_OPEN, AnchoredComment, parse_anchored_comments};
use crate::{
...
pub(crate) fn build_export_text(session_id: &str, hunks: &[HunkView]) -> String {
    ...
    for hunk in hunks.iter().filter(|h| !h.comment.trim().is_empty()) {
        ...
        for (comment_index, entry) in anchored.into_iter().enumerate() {
            ...
            out.push('\n');
            out.push_str("Run this when done: ");
            out.push_str(&format!(
                "curl -fsS -H \"Authorization: Bearer $({} generate-pass-key)\" {}/api/session/{session_id}/resolve/{}/{comment_index}",
                shell_quoted_moon(),
                export_server_url(),
                hunk.id
            ));
...
/// This executable, quoted for a shell: how the notes' agent gets the pass key the resolve
/// link needs - see [`crate::pass_keys`]. Asked for when the agent runs it rather than written
/// into the notes, so no key sits in the notes, the agent's transcript or its logs. The agent
