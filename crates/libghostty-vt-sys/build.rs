...
fn build_vendored(link_mode: LinkMode) {
    ...
    // Ghostty's wasm build installs its libraries but not its headers; they are the source's.
    if !include_dir.join("ghostty").join("vt.h").exists() && target.starts_with("wasm32") {
        copy_dir(&ghostty_dir.join("include"), &include_dir);
    }
    assert!(
...
/// Copy a folder and everything under it - Ghostty's headers, for a wasm build that installed
/// none. See VENDORED.md.
fn copy_dir(from: &Path, to: &Path) {
    std::fs::create_dir_all(to).expect("could not make the include folder");
    for entry in std::fs::read_dir(from).expect("could not read Ghostty's include folder") {
        let entry = entry.expect("could not read an include entry");
        let target = to.join(entry.file_name());
        if entry.path().is_dir() {
            copy_dir(&entry.path(), &target);
        } else {
            std::fs::copy(entry.path(), &target).expect("could not copy a header");
        }
    }
}
